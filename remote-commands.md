...
3. (Pushing a new branch) You commit some files to the `dev-foo` branch and try to "push" them to Github, but it fails as shown here:
   ...
   ```
   Explain this error.
   > occurs when you try to execute the git push command without specifying the remote branch you want to push to and without setting up an upstream branch for your current local branch.
